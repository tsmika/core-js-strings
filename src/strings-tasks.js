...
function formatTime(minutes, seconds) {
  const mins = minutes < 10 ? String(minutes).padStart(2, '0') : minutes;
  const secs = seconds < 10 ? String(seconds).padStart(2, '0') : seconds;
...
function isPalindrome(str) {
  const strToCompare = str.toLowerCase().replace(/[\s,!?]/g, '');
  return strToCompare === strToCompare.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((elem) => {
    if (elem.split('').length > result.split('').length) result = elem;
  });
  return result;
}
...
function reverseWords(str) {
  ...
  str.split(' ').forEach((elem) => {
    result += elem.split('').reverse().join('').concat(' ');
  });
...
function invertCase(str) {
  return str.replace(/([a-z]+)|([A-Z]+)/g, (match, lowerCase, upperCase) => {
    if (lowerCase) {
...
function encodeToRot13(str) {
  ...
  let result = '';
  str.split('').forEach((elem) => {
    if (SOURCE.includes(elem)) {
      result += TARGET.charAt(SOURCE.indexOf(elem));
    } else {
      result += elem;
    }
  });
...
function getCardId(value) {
  ...
  for (let i = 0; i < CARD_DECK.length; i += 1) {
    if (CARD_DECK[i] === value) return i;
  }
  return undefined;
}
